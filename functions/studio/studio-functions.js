...
function reverseCharacters(reversedString) {
    if (typeof reversedString === 'string') {
        let lettersArray = reversedString.split('');
        let reversedLettersArray = lettersArray.reverse();
        return reversedLettersArray.join('');
    } else {
        let reversedNumbers = String(reversedString).split('').reverse().join('');
        return Number(reversedNumbers);

    }
}
let holdThisString = 'radar';

console.log(reverseCharacters(holdThisString));
...
function completeReversal(reversingArr) {
    let startingArr = [];
